...
func (user WxUser) YesterdayRegisterUser() int {
	now := time.Now()
	...
	var err = DB.Model(&WxUser{}).Where("created_at >= ? AND created_at < ?", yesterdayYMD, todayYMD).Count(&count).Error
	if err != nil {
		return 0
	}
	return count
...
func (user WxUser) TodayRegisterUser() int {
	now := time.Now()
	today := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.Local)
	todaySec := today.Unix()           //秒
	...
	var count int
	var err = DB.Model(&WxUser{}).Where("created_at >= ? AND created_at < ?", todayYMD, tomorrowYMD).Count(&count).Error
	if err != nil {
		return 0
	}
	return count
...
func (user WxUser) PurchaseUserByDate(date time.Time) int {
